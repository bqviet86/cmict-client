...
function Introduce() {
    ...
    const getAllPostsIntroductionQueryFn = async ({ page, limit }: { page: number; limit: number }) => {
        ...
        navigate(`${routes.introduce}?page=${page}`)

        return response
    }

    const { isFetching } = useQuery({
        queryKey: ['allPostsIntroduction', { page: pagination.page, limit: LIMIT }],
        queryFn: () =>
            getAllPostsIntroductionQueryFn({
    ...
    return (
        <Panel title='Giới thiệu' color='#e74c3c'>
            {isFetching ? (
