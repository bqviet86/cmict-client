...
function Services() {
    ...
    const getAllPostsServiceQueryFn = async ({ page, limit }: { page: number; limit: number }) => {
        ...
        navigate(`${routes.services}?page=${page}`)

        return response
    }

    const { isFetching } = useQuery({
        queryKey: ['allPostsService', { page: pagination.page, limit: LIMIT }],
        queryFn: () =>
            getAllPostsServiceQueryFn({
    ...
    return (
        <Panel title='Dịch vụ' color='#e74c3c'>
            {isFetching ? (
